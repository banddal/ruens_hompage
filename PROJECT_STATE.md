...
- **라운드:** 2026-06-29 (12차) / 작성: Claude
- **한 줄:** essay 링크 추출 — 브런치 안 되던 원인(모바일UA 고정) 발견 + 네이버/브런치 버튼 분리·UA 자동선택.
- **⚠️ 진행중. 매 작업마다 갱신.**
- **문제:** essay 메타추출이 네이버는 되는데 브런치 안 됨. 원인 = fetchMetaFromUrl의 User-Agent가
  **모바일(iPhone) 고정**(이전 네이버 뚫으려 변경한 잔재). 브런치는 데스크톱 UA라야 메타 반환.
- **해결:**
  - 백엔드: UA_DESKTOP/UA_MOBILE 상수화. fetchMetaFromUrl(url, ua) UA 인자화.
    fetchEssayMetadata(url, source): 사이트감지(브런치=데스크톱우선, 네이버=모바일우선) + 양쪽 UA 폴백 시도.
    handleEssayFetchMeta가 payload.source 전달. 일괄등록은 host자동감지.
  - admin: 버튼 1개(fetchMetaBtn) → **2개 분리**(#fetchMetaNaver source=naver, #fetchMetaBrunch source=brunch).
    fetchEssayMeta(source) 인자화. admin.css 버튼 스타일(초록).
- **검증:** node --check 통과. Playwright 두 버튼 렌더 확인(네이버/브런치), 에러없음.
  ※실제 추출성공은 배포 후 확인(샌드박스는 외부사이트 차단으로 미검증).
- **사용자 액션:** api/index.js+admin 3개 교체 push. 캐시 brunch-1. 배포 후 브런치 링크+브런치버튼으로 추출되는지 확인.
