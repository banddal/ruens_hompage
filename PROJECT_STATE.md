...
- **라운드:** 2026-06-29 (6차) / 작성: Claude
- **한 줄:** G-FAIR 가짜런타임분할 제거 + 진짜 3개 레코드 등록 기능(admin "G-FAIR 등록" 버튼) 구축.
- **⚠️ 진행중. 매 작업마다 갱신.**
- **문제였던 것:** Codex가 G-FAIR를 `withGfairKoreaSplit()`로 **런타임에 1개(legacy gfair)→3개로 가짜분할**.
  프론트엔 3개 보였으나 admin은 가짜라 자료 업로드 불가(실제 DB레코드 없음).
- **해결:**
  - api/index.js: `withGfairKoreaSplit` 함수+호출 4곳 **완전 제거**. getProjectsStore가 Supabase/시드 그대로 반환.
  - 시드(content-data/projects.json)·data.js엔 이미 g-fair-korea-2021/2022/2023-pm 3개 정상 존재.
  - **신규 핸들러 handleGfairSync + 라우트 POST /api/admin/projects/sync-gfair**: 시드의 G-FAIR 3개를
    Supabase에 upsert(이미 있으면 자료보존 위해 건너뜀) + 레거시 `gfair` 레코드 삭제.
  - admin.html: Projects 패널에 **"G-FAIR 등록" 버튼**(#syncGfair) 추가. admin.js: 클릭→sync→목록갱신.
    admin.css: 버튼 빨강 스타일. 캐시 `gfair-2`.
- **검증:** node --check api/index.js·admin.js 통과. admin 버튼 Playwright 렌더 확인(에러 없음).
- **사용자 액션 필요:** 배포 후 admin Projects 탭 → **"G-FAIR 등록" 버튼 1회 클릭** → 3개 등록됨 →
  각각 자료(이미지·파일) 업로드 가능. (스키마 변경 없음, Supabase 재실행 불필요)
