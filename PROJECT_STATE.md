...
- **라운드:** 2026-06-29 (11차) / 작성: Claude
- **한 줄:** admin 이미지 — ①100개초과 사라짐 버그수정(페이지네이션) ②다중선택삭제+드래그순서변경 추가.
- **⚠️ 진행중. 매 작업마다 갱신.**
- **(1) 이미지 사라짐 버그 수정:** 원인 = supabaseListProjects가 project_images 전체를 `select=*` 한 번에 로드
  → Supabase REST 행제한(기본~1000, 환경따라 100)에 걸려 초과분 잘림. 업로드시 메모리엔 보이나 재조회시 사라짐.
  해결: `supabaseRequestAll()` 신규 — Range 헤더로 1000씩 페이지네이션, 416 tolerant, 최대 50p. projects/images/files 전부 적용.
- **(2) 이미지 관리 기능(이미지 한정):**
  - 다중선택: 체크박스 + 전체선택 + "선택 삭제" → POST /api/admin/projects/:id/images/bulk-delete (id=in.() 일괄삭제).
  - 순서변경: 드래그앤드롭(⠿ 핸들) → POST /api/admin/projects/:id/images/reorder (sort_order 순차 PATCH).
  - admin.html: imageBulkBar 툴바. admin.js: setupImageBulkTools(체크·전체선택·벌크삭제·DnD). admin.css 스타일.
  - 백엔드: bulkDeleteImages, reorderImages 핸들러 + 라우트(POST, assetDelete[DELETE]와 메서드달라 충돌없음).
- **검증:** node --check 통과. Playwright mock 3이미지 → 체크박스3·핸들3·전체선택·선택삭제·draggable3 정상, 에러없음.
- **사용자 액션:** api/index.js+admin 3개 교체 push. 스키마 변경 없음. 캐시 imgtools-1.
  배포 후 100개 넘는 프로젝트에서 이미지 다 보이는지 + 선택삭제/드래그순서 확인.
- **바뀐 파일:** api/index.js, admin.html, admin.js, admin.css.
