...
- **라운드:** 2026-06-29 (13차) / 작성: Claude
- **한 줄:** 새 프로젝트 "민건 8기 경기도 정책 제안" 추가(S2/2022/Side/Plan) — 타임라인+시드+data.js+SQL 4곳 연결.
- **⚠️ 진행중. 매 작업마다 갱신.**
- **추가 내용:** id=mingeon8-gyeonggi-policy, category=Plan, period=2022, 위치=S2 타임라인 2022 Side Project 레인.
  세부내용(설명·역할·outcome)은 빈 상태 → 사용자가 admin에서 채울 예정.
- **4곳 연결:**
  ① index.html 타임라인: 2022 Side 빈 셀(year-cell empty)에 js-open-project 버튼 추가.
  ② content-data/projects.json: 시드에 추가(30→31개). ③ data.js PROJECTS 배열에 추가(하드코딩 폴백).
  ④ add-mingeon8-project.sql: Supabase insert(tags/skill_tags 제외=타입회피, on conflict do nothing).
  ※archiving·admin은 프로젝트목록 자동읽기라 SQL만 넣으면 자동 표시(별도작업 불필요).
- **검증:** Playwright — 타임라인 버튼 렌더, 클릭→모달 열림(제목정확), archiving에 텍스트 존재, 에러없음. node--check 통과.
- **사용자 액션:** index.html+data.js+projects.json 교체 push + add-mingeon8-project.sql 1회 실행.
  이후 admin에서 해당 프로젝트 열어 설명·이미지 등 채우기.
- **바뀐 파일:** index.html, data.js, content-data/projects.json, add-mingeon8-project.sql(신규).
...
- 2026-06-29 (13차) · Claude · 새 프로젝트 "민건 8기 경기도 정책 제안" 추가(타임라인2022Side+시드+data.js+SQL).
