...
- **라운드:** 2026-06-29 (14차) / 작성: Claude
- **한 줄:** archiving Portfolio 블록이 data.js만 읽어 admin(Supabase) 변경 미반영 버그 수정 — hydrate후 갱신 연결.
- **⚠️ 진행중. 매 작업마다 갱신.**
- **문제:** archiving Portfolio(renderPortfolio)는 PROJECTS(data.js 하드코딩)만 읽음. 민선8기 등 빈상태 프로젝트는
  admin(Supabase)에서 내용 채워도 archiving Portfolio엔 빈값 그대로 표시. hydrateProjectCache는 개별카드·대시보드만
  갱신하고 PROJECTS/archiving Portfolio는 안 건드림.
- **해결:** ① initArchiving에서 window.refreshArchPortfolio = renderPortfolio 노출.
  ② hydrateProjectCache가 Supabase projects를 PROJECTS에 머지(있으면 갱신, 없으면 추가) 후 refreshArchPortfolio("all") 호출.
  → admin에서 채운 내용이 archiving Portfolio에도 반영됨. (essay의 refreshArchEssays와 동일 패턴)
- **검증:** Playwright — PROJECTS에 short/metric 채우고 refreshArchPortfolio 호출 → archiving 행 텍스트 갱신 확인.
- **사용자 액션:** app.js+index.html 교체 push. 캐시 archpf-1. 배포 후 admin에서 민선8기 내용 채우면 archiving Portfolio 반영 확인.
- **바뀐 파일:** app.js, index.html.
- **참고:** 민선8기 프로젝트 = 타임라인2022Side+archiving+admin+Supabase+CV리더십 5곳 연결완료. 내용은 사용자가 admin에서 채울 것.
