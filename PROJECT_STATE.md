...
- **라운드:** 2026-06-29 (10차) / 작성: Claude
- **한 줄:** archiving 에세이 클릭 무반응 **진짜원인=스와이프 포인터캡처** 발견·수정. 콘솔진단으로 확정.
- **⚠️ 진행중. 매 작업마다 갱신.**
- **확정 진단(사용자 콘솔):** 데이터/항목/createEssayCard/본문 전부 정상(28항목, body 7821자).
  즉 데이터연결 문제 아님. dispatchEvent(코드클릭)는 모달 열림, 실제 마우스클릭만 안 열림.
- **진짜원인:** enableArchEssayBoardSwipe가 **pointerdown 즉시 setPointerCapture** 호출 →
  단순 클릭(이동0)에도 포인터가 board에 캡처돼 click 이벤트가 위임핸들러(#archEssayList)에 정상 도달 못함.
  essay 메인탭은 이 스와이프 없어서 잘 열렸던 것.
- **해결:** setPointerCapture를 pointerdown에서 제거 → **pointermove에서 4px 이상 실제 드래그 감지시에만 캡처**.
  단순 클릭은 캡처 안 함 → 클릭 정상 통과 → 모달 열림. (8·9차의 이벤트위임/refreshArchEssays도 유지)
- **검증:** Playwright 실제 마우스클릭(스크롤로 항목 화면내 배치 후)→모달 open:True. elementFromPoint=항목자식(덮개없음).
- **바뀐 파일:** app.js(enableArchEssayBoardSwipe 수정), index.html(캐시 archmodal-3).
- **교훈:** Playwright 클릭은 항목이 뷰포트 밖이면 빗나감 → scroll_into_view 필수. "코드클릭은 되는데 실제클릭 안 됨"=덮개/캡처 의심.
