...
- **라운드:** 2026-06-29 (4차) / 작성: Codex
- **한 줄:** 에세이 댓글 시스템(localStorage→Supabase) 연결 완료. 공개 댓글/답글/자가삭제 + 관리자 Comments/IP차단 UI까지 반영.
- **완료 (Claude):**
  - 스키마: `essay_comments`(답글=parent_id 중첩, password_hash=sha256, source_ip, is_blocked)
    + `blocked_ips`(IP차단). supabase-schema.sql 끝. RLS: 차단안된 댓글만 공개 SELECT, 쓰기 service_role.
  - 백엔드 핸들러 13개 함수: handleCommentList/Create/SelfDelete,
    handleAdminCommentList/Delete, handleBlockIp/UnblockIp/BlockedIpList, buildCommentTree, isIpBlocked,
    hashCommentPassword. 스팸방어(허니팟+입력제한+속도제한) + IP차단 확인 내장.
  - **버그 수정:** 기존 hashPassword(프로젝트 인증용)와 이름 충돌 → 댓글용을 hashCommentPassword로 분리.
- **완료 (Codex):**
  - `api-index.js`에 댓글 라우트 등록 후 실제 배포 파일 `api/index.js`에 동일 반영.
    `/api/essays/:id/comments`는 일반 `/api/essays/:id`보다 먼저 매칭되도록 배치.
  - `app.js` 댓글을 localStorage 기반에서 Supabase API 기반으로 전환.
    댓글/답글 작성, parentId 기반 중첩, 삭제용 비밀번호 자가삭제, 허니팟 필드 적용.
  - `index.html` 댓글 폼에 삭제용 비밀번호와 허니팟 필드 추가.
  - `admin.html/admin.js/admin.css`에 Comments 탭 추가.
    댓글 목록, 댓글 삭제, IP 차단, 차단 IP 목록, 차단 해제 연결.
  - 캐시버스터 `20260629-comment-1` 적용.
- **결정사항:** 익명+간단비번(자가삭제용), 답글 유지, 관리자 삭제+IP차단+스팸방어 전부.
- **검증:** `node --check api/index.js`, `node --check api-index.js`, `node --check app.js`, `node --check admin.js` 통과.
  `api-index.js`와 `api/index.js` SHA256 동일 확인.
- **바뀐 파일:** `api-index.js`, `api/index.js`, `app.js`, `index.html`, `admin.html`, `admin.js`, `admin.css`, `styles.css`.
- **남은 사람 작업:** 배포 전 Supabase SQL Editor에서 최신 `supabase-schema.sql` 재실행 필요.
- **별건(미착수):** essay/portfolio "수정 버튼" 명확화 — 수정은 이미 작동(on_conflict merge)하나 UI 체감 부족.
  Codex 시간되면, 아니면 Claude가 이어받음.
